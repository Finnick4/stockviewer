...
function showModalEditStockGroup(groupid) {
    let groupName, groupDescription, groupMembers
    ...
    let html = `<h2>${getTranslatedStr("stockgroups.modify.title_edit", {name: getTranslatedStr("stocks.modify.loading_name")})}</h2>
                    ${permName ? `
                    <div class="pair">
    ...
    fetch(`/api/stockgroups/${groupid}`).then(r => r.json()).then(resp => {
        groupDescription = sanitiseText(resp.Description)
        description.innerHTML = groupDescription
    })

    loadStockGroupData(groupid, data => {
        groupName = sanitiseText(data.Name)
        groupMembers = data.Members

        modal.querySelector("h2").innerHTML = getTranslatedStr("stockgroups.modify.title_edit", {name: groupName})
